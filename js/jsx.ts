import type { cleanup, Running, watch, watchFn } from "~/signals";
...
export function template(html: string): () => Node {
  const createNode = (): Node => {
    ...
    if (error) {
      throw new Error("Invalid XML in template string");
    }

    const xmlNode = xml.documentElement.firstChild;
    if (!xmlNode) {
      throw new Error("No valid nodes found in template string");
    }

    return document.importNode(xmlNode, true);
  };

  let cache: Node | undefined;
  // biome-ignore lint/suspicious/noAssignInExpressions:
  return (): Node => (cache || (cache = createNode())).cloneNode(true);
}
